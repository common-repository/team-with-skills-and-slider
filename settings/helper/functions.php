<?php
function twss_6310_replace($data) {
	if($data == '') return ''; 
...
function twss_6310_extract_contact_info($data, $ids){
	$str = "";
	if ($data) {
		...
		if($contacts){
			foreach ($contacts as $contact) {
				...
				if (filter_var($contact1, FILTER_VALIDATE_EMAIL)) {
					$dataAttr = "twss-data-custom-field='mailto:{$contact1}'";
					$dataClass = ' twss-6310-custom-field-mail-link-class';
				} else if (filter_var($contact1, FILTER_VALIDATE_URL)) {
					$contact1 = twss_6310_validate_profile_url($contact1);
					$dataAttr = "twss-data-custom-field='{$contact1}'";
					$dataClass = ' twss-6310-custom-field-mail-link-class';
				} 
				$str .= "<div class='twss-custom-fields-list-{$ids}{$dataClass}' {$dataAttr}><div class='twss-custom-fields-list-label-{$ids}'>" . str_replace("\\", "", $contact[0]) . "</div> <div class='twss-custom-fields-list-content-{$ids}'>{$contact[1]}</div></div>";
			}    
		}       
	}
	if($str){
		$str = "<div class='twss-custom-fields-{$ids}'>{$str}</div>";
	}
	echo $str;
}


function twss_6310_external_link($data)
{
	$data = trim($data);
	if (filter_var($data, FILTER_VALIDATE_EMAIL)) {
		...
		$data = " tooltip-href='{$data}' twss-6310-tooltip='yes'  class='open_in_new_tab_class twss-6310-tooltip' ";
	}
	return $data;
}

function twss_6310_get_user_roles(){
...
function twss_6310_search_template($ids, $cssData){
	?>
	<div class="twss-6310-search-<?php echo $ids ?>">
		<div class="twss-6310-search-container twss-6310-search-template-<?php echo $ids ?>">
			<input type="text" name="twss-6310-search-box"  data-twss-6310-template-id='<?php echo $ids ?>' class="twss-6310-search-box" autocomplete="off" placeholder="<?php echo (isset($cssData['search_placeholder']) && $cssData['search_placeholder'] !== '') ? $cssData['search_placeholder'] : '' ?>">
	...
	<style>
		.twss-6310-search-<?php echo $ids ?> {
			display: flex;
			justify-content: <?php echo (isset($cssData['search_align']) && $cssData['search_align'] !== '') ? $cssData['search_align'] : 'flex-start' ?>;
			margin: 0 <?php echo (isset($cssData[127])?$cssData[127]:15) ?>px;
			width: calc(100% - <?php echo (isset($cssData[127])?$cssData[127]:15) * 2 ?>px) !important;
		}

		.twss-6310-search-template-<?php echo $ids ?> {
			display: <?php echo (isset($cssData['search_activation']) && $cssData['search_activation']) ? 'flex' : 'none' ?>;
			position: relative;
			width: calc(33% - 15px);
			margin-bottom: <?php echo (isset($cssData['search_margin_bottom']) && $cssData['search_margin_bottom'] !== '') ? $cssData['search_margin_bottom'] : 10 ?>px;
		}
		.twss-6310-search-template-<?php echo $ids ?> input {
			width: 100% !important;
			border: <?php echo (isset($cssData['search_border_width']) && $cssData['search_border_width'] !== '') ? $cssData['search_border_width'] : '2' ?>px solid <?php echo (isset($cssData['search_border_color']) && $cssData['search_border_color'] !== '') ? $cssData['search_border_color'] : '#000' ?>;
			border-radius: <?php echo (isset($cssData['search_border_radius']) && $cssData['search_border_radius'] !== '') ? $cssData['search_border_radius'] : '50' ?>px;
			padding: 5px 40px 5px 12px;
			color: <?php echo (isset($cssData['search_font_color']) && $cssData['search_font_color'] !== '') ? $cssData['search_font_color'] : '#000'?>;
			height: <?php echo (isset($cssData['search_height']) && $cssData['search_height'] !== '') ? $cssData['search_height'] : '40';
			?>px;
			line-height: <?php echo (isset($cssData['search_height']) && $cssData['search_height'] !== '') ? $cssData['search_height'] : '40';
			?>px;
			font-size: 15px;
			transition: all 0.3s;
		}
		.twss-6310-search-template-<?php echo $ids ?> input::placeholder {
			color: <?php echo (isset($cssData['search_placeholder_font_color']) && $cssData['search_placeholder_font_color'] !== '') ? $cssData['search_placeholder_font_color'] : 'rgb(128, 128, 128)' ?>;
		}
		.twss-6310-search-template-<?php echo $ids ?> input:focus {
			outline: none !important;
			box-shadow: none !important;
			border-color: <?php echo (isset($cssData['search_border_color']) && $cssData['search_border_color'] !== '') ? $cssData['search_border_color'] : '#000' ?> !important;
		}
		.twss-6310-search-template-<?php echo $ids ?> i.search-icon {
			position: absolute;
			right: 12px;
			top: 50%;
			transform: translateY(-50%);
			font-size: 14px;
			color: <?php echo (isset($cssData['search_border_color']) && $cssData['search_border_color'] !== '') ? $cssData['search_border_color'] : '#000' ?>;
		}
...
?>
   <div class="twss_6310_member_skills_wrapper_<?php echo $output; ?>">
      ...
      foreach ($skills as $skills) {
         $skills = explode("||||", $skills);
      ?>
         <div class="twss_6310_skills_label_<?php echo $output; ?>"><?php echo twss_6310_replace($skill[0]) ?></div>
         <div class="twss_6310_skills_prog_<?php echo $output; ?>">
            <div class="twss_6310_fill_<?php echo $output; ?> fill-<?php echo $id . "-{$skl}-$output" ?>" data-progress-animation="<?php echo $skills[1] ?>%" data-appear-animation-delay="400" style="width: <?php echo twss_6310_replace($skill[1]) ?>%;"></div>
         </div>
         <style>
  ...
  <div class="twss_6310_fill_ fill--3-" data-progress-animation="100%" data-appear-animation-delay="400" style="width: 100%;"></div>
   ...
   ?>
      <script>
         ...
         jQuery("body").on("change", "#skills_font_size", function() {
            jQuery("<style type='text/css'>.twss_6310_skills_label_<?php echo $id ?>{ font-size:" + jQuery(this).val() +
               "px !important;} </style>").appendTo(".twss-preview-box");
         });

         jQuery("body").on("change", "#skills_text_transform", function() {
            jQuery("<style type='text/css'>.twss_6310_skills_label_<?php echo $id ?> { text-transform:" + jQuery(this).val() +
               ";} </style>").appendTo(".twss_6310_tabs_panel_preview");
         });

         jQuery("body").on("change", "#skills_font_color", function() {
            jQuery("<style type='text/css'>.twss_6310_skills_label_<?php echo $id ?> { color:" + jQuery(this).val() +
               ";} </style>").appendTo(".twss_6310_tabs_panel_preview");
         });
         ...
         jQuery("body").on("change", "#skills_font_hover_color", function() {
            ...
            jQuery(
               "<style type='text/css'>.twss_6310_team_style_<?php echo $id; ?>:hover .twss_6310_skills_label_<?php echo $id ?>{ color:" +
               value + ";} </style>").appendTo(".twss_6310_tabs_panel_preview");
         });

         jQuery("body").on("change", "#skills_font_weight", function() {
            jQuery("<style type='text/css'>.twss_6310_skills_label_<?php echo $id ?> { font-weight:" + jQuery(this).val() +
               ";} </style>").appendTo(".twss_6310_tabs_panel_preview");
         });
         ...
         jQuery("body").on("change", "#skills_font_family", function() {
            ...
            value = value.split(':');
            jQuery("<style type='text/css'>.twss_6310_skills_label_<?php echo $id ?> { font-family:" + value + ";} </style>")
               .appendTo(".twss_6310_tabs_panel_preview");
         });

         jQuery("body").on("change", "#skills_line_height", function() {
            jQuery("<style type='text/css'>.twss_6310_skills_label_<?php echo $id ?> { line-height:" + jQuery(this).val() +
               "px;} </style>").appendTo(".twss_6310_tabs_panel_preview");
         });

         jQuery("body").on("change", "#progress_bar_height", function() {
            jQuery("<style type='text/css'>.twss_6310_skills_prog_<?php echo $id ?> { height:" + jQuery(this).val() +
               "px;} </style>").appendTo(".twss_6310_tabs_panel_preview");
         });

         jQuery("body").on("change", "#progress_bar_border_radius", function() {
            var value = jQuery(this).val() + "px";
            jQuery("<style type='text/css'>.twss_6310_skills_prog_<?php echo $id ?> { border-radius:" + value +
               "; -moz-border-radius:" + value + "; -ms-border-radius:" + value + "; -o-border-radius:" + value +
               "; -webkit-border-radius:" + value + ";} </style>").appendTo(".twss_6310_tabs_panel_preview");
         });

         jQuery("#progress_bar_border_size").on("change", function() {
            var value = jQuery(this).val() + "px";
            jQuery("<style type='text/css'>.twss_6310_skills_prog_<?php echo $id ?> { border-width:" + value + ";} </style>")
               .appendTo(".twss_6310_tabs_panel_preview");
         });

         jQuery("#progress_bar_border_color").on("change", function() {
            var value = jQuery(this).val();
            jQuery("<style type='text/css'>.twss_6310_skills_prog_<?php echo $id ?>  { border-color:" + value + ";} </style>")
               .appendTo(".twss_6310_tabs_panel_preview");
         });

         jQuery("#progress_bar_background_color").on("change", function() {
            var value = jQuery(this).val();
            jQuery("<style type='text/css'>.twss_6310_skills_prog_<?php echo $id ?>  { background-color:" + value +
               ";} </style>").appendTo(".twss_6310_tabs_panel_preview");
         });

         jQuery("#twss_progress_bar_margin_top").on("change", function() {
            var value = jQuery(this).val() + "px";
            jQuery("<style type='text/css'>.twss_6310_member_skills_wrapper_<?php echo $id ?> { margin-top:" + value +
               ";} </style>").appendTo(".twss_6310_tabs_panel_preview");
         });
         jQuery("#twss_progress_bar_margin_bottom").on("change", function() {
            var value = jQuery(this).val() + "px";
            jQuery("<style type='text/css'>.twss_6310_member_skills_wrapper_<?php echo $id ?>{ margin-bottom:" + value +
               ";} </style>").appendTo(".twss_6310_tabs_panel_preview");
         });
         ...
         jQuery("#progress_bar_type, #progress_bar_color, #progress_bar_alternate_color, #progress_bar_animation").on("change",
            function() {
               ...
               if (pbt) {
                  ...
                  if (pb_animation) {
                     jQuery(
                        "<style type='text/css'>.twss_6310_fill_<?php echo $id ?>  { animation-iteration-count: infinite !important;} </style>"
                     ).appendTo(".twss_6310_tabs_panel_preview");
                  } else {
                     jQuery(
                        "<style type='text/css'>.twss_6310_fill_<?php echo $id ?>  { animation-iteration-count: 0 !important;} </style>"
                     ).appendTo(".twss_6310_tabs_panel_preview");
                  }

                  //Set Progress Bar Color
                  jQuery("<style type='text/css'>.twss_6310_fill_<?php echo $id ?>  { background-color:" + pb_color +
                     ";} </style>").appendTo(".twss_6310_tabs_panel_preview");
                  ...
                  jQuery("<style type='text/css'>.twss_6310_fill_<?php echo $id ?>  { background-image:" + pb_alt_color +
                     ";} </style>").appendTo(".twss_6310_tabs_panel_preview");
                  ...
                  jQuery("<style type='text/css'>.twss_6310_fill_<?php echo $id ?>  { background-image:" + pb_color +
                     ";} </style>").appendTo(".twss_6310_tabs_panel_preview");
               }
            });
      </script>
   ...
   ?>
      <style>
         ...
         .twss_6310_fill_<?php echo $id; ?> {
            ...
            <?php if ($cssData['progress_bar_type']) {
   ...
   function twss_6310_team_member_details()
   {
      ...
      if ($data['styledata']['iconids']) {
         $icons = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}twss_6310_icons WHERE id in (%s)", $data['styledata']['iconids']), ARRAY_A);
         if ($icons) {
            ...
            foreach ($icons as $li) {
               ...
               $html .= "<a '".twss_6310_external_link($iconUrl[$index])."' class='twss-popup-link' title='{$li['name']}' target='_blank' id='twss-social-link-{$ids}-{$li['id']}'><i class='" . $li['class_name'] . "'></i></a>";
               $iconStyles .= "<style>#twss-social-link-{$ids}-{$li['id']}{border: {$cssData['social_border_width']}px solid {$li['bgcolor']}; background-color: {$li['bgcolor']}; color:{$li['color']};} #twss-social-link-{$ids}-{$li['id']}:hover{background-color: {$li['color']}; color:{$li['bgcolor']};} </style>";
            }
   ...
   function twss_6310_team_member_info()
   {
      ...
      while($found){
         $temp = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}twss_6310_style WHERE id = %d ", $styleId), ARRAY_A);
      ...
      $data['styledata']['name'] = twss_6310_replace($data['styledata']['name']);
      $data['styledata']['designation'] = twss_6310_replace($data['styledata']['designation']);
      $data['styledata']['profile_details'] = str_replace("\'", "'", $data['styledata']['profile_details']);
      $data['styledata']['profile_details'] = str_replace('\"', '"', $data['styledata']['profile_details']);
         ...
         if ($iconIds && $iconUrl) {
            for ($i = 0; $i < count($iconUrl); $i++) {
               if ($iconIds[$i] != "" && $iconUrl[$i] != "") {
                  $selIcon = $wpdb->get_row($wpdb->prepare("SELECT * FROM $icon_table WHERE id=%d", $iconIds[$i]), ARRAY_A);
                  if ($selIcon) {
                     $html .= "<a ".twss_6310_external_link($iconUrl[$i])." data-social-modal='1'  title='{$selIcon['name']}'  id='twss-social-link-{$ids}-{$selIcon['id']}'><i class='" . $selIcon['class_name'] . "'></i></a>";
                     $iconStyles .= "<style>#twss-social-link-{$ids}-{$selIcon['id']}{border: 2px solid {$selIcon['bgcolor']}; background-color: {$selIcon['bgcolor']}; color:{$selIcon['color']};} #twss-social-link-{$ids}-{$selIcon['id']}:hover{color: {$selIcon['bgcolor']}; background-color:{$selIcon['color']};} </style>";
                  }
               }
            }
            $data['link'] = $html . $iconStyles;
      ...
      $str = '';
      if ($data['styledata']['contacts']) {
         $contacts = explode("####||||####", $data['styledata']['contacts']);
         if($contacts){
            foreach ($contacts as $contact) {
               $contact = explode("||||", $contact);
               $contact1 = trim($contact[1]);
               ...
               $labelText = str_replace("\\", "", $contact[0]);
               ...
               $str .= "<div class='twss-custom-fields-list{$dataClass}' {$dataAttr}><div class='twss-custom-fields-list-label'>" . $labelText ."</div> <div class='twss-custom-fields-list-content'>:{$contact[1]}</div></div>";
            }    
         }   
      }
   ...
   function twss_6310_validate_profile_url($url){
...
// Output Functions
   ...
   function twss_fun_template_name($value, $id, $template_id = 0)
   {
      ...
      echo twss_6310_replace("<div class='twss_6310_team_style_{$id}_title'>{$value}</div>");
   }

   function twss_fun_template_designation($value, $cssData, $id, $template_id = 0)
   {
      $id = $template_id ? $template_id : $id;
      echo twss_6310_replace("<div class='twss_6310_team_style_{$id}_designation'>{$value}</div>");
   ?>
      <script>
         <?php
            if(!$cssData['twss_fun_template_designation']){
               echo "jQuery('.designation_act_field').hide();";
            }
         ?>
         jQuery("body").on("click", ".activate-designation", function() {
            ...
            if (val == 0) {
               jQuery(".designation_act_field, .twss_6310_team_style_<?php echo $id; ?>_designation").hide();
            } else {
               jQuery(".designation_act_field, .twss_6310_team_style_<?php echo $id; ?>_designation").show();
            }
         });
      </script>
   <?php
   ...
   function twss_fun_template_profile_details($value, $cssData, $id, $template_id = 0)
   {
      ...
      if ($value['profile_details_type'] == 2) {
         echo "<div class='twss_6310_team_style_{$id}_profile_details'>" .
            twss_my_word_count($value['profile_details'], $cssData['details_summary_words'])
            . "</div>";
      }
   ?>
      <script>
         <?php
            if(!$cssData['twss_fun_template_profile_details']){
               echo "jQuery('.details_act_field').hide();";
            }
         ?>
         jQuery("body").on("click", ".activate-details", function() {
            ...
            if (val == 0) {
               jQuery(".details_act_field, .twss_6310_team_style_<?php echo $id; ?>_profile_details").hide();
            } else {
               jQuery(".details_act_field, .twss_6310_team_style_<?php echo $id; ?>_profile_details").show();
            }
         });
      </script>
   <?php
   ...
   ?>
      <style>
         ...
         .twss_6310_team_style_<?php echo $ids; ?>_read_more div {
            ...
            height: <?php echo ($cssData['read_more_height'] + ((int) $cssData['read_more_border_width'] * 2)) ?>px;
            line-height: <?php echo $cssData['read_more_height'] ?>px;
            letter-spacing: 0.06em;
            font-family: <?php echo str_replace("+", " ", $cssData['read_more_font_family']); ?>;
            ...
            width: <?php echo $cssData['read_more_width'] ?>px;
            text-align: center;
            cursor: pointer;
            ...
            <?php if ($cssData['read_more_text_align'] == 'center') {
               echo "margin: 0 auto;";
         ...
         .twss_6310_team_style_<?php echo $ids; ?>_read_more div:hover{
            ...
            border: <?php echo $cssData['read_more_border_width'] ?> solid <?php echo $cssData['read_more_border_hover_color'] ?>;
         }
      </style>
   <?php
   }
   ...
   ?>
      <script>
         <?php
               if(!$cssData['twss_fun_template_read_more']){
         ...
         jQuery("#twss_read_more_text").on("input", function() {
            ...
            $(".twss_6310_team_style_<?php echo $ids; ?> .twss_6310_team_style_<?php echo $ids; ?>_read_more div").text(
               value);
         });

         jQuery("#twss_read_more_height, #twss_read_more_border_width").on("change", function() {
            var read_more_height = parseInt(jQuery('#twss_read_more_height').val()) ;
            var  read_more_border_width = parseInt(jQuery('#twss_read_more_border_width').val()) ;
            var read_more_line_height = Math.ceil(read_more_height + read_more_border_width * 2) + "px";

            jQuery(
               "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?> .twss_6310_team_style_<?php echo $ids; ?>_read_more div  { height:" + read_more_line_height
                   + ";} </style>").appendTo(".twss-6310-preview");
         });
         jQuery("#twss_read_more_height").on("change", function() {
            var value = jQuery(this).val() + "px";
            jQuery(
               "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?> .twss_6310_team_style_<?php echo $ids; ?>_read_more div  {line-height:" +
               value + ";} </style>").appendTo(".twss-6310-preview");
         });

         jQuery("#twss_read_more_width").on("change", function() {
            var value = jQuery(this).val();
            jQuery(
               "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?> .twss_6310_team_style_<?php echo $ids; ?>_read_more div  { width:" +
               value + "px;} </style>").appendTo(".twss-6310-preview");
         });

         jQuery("#twss_read_more_font_size").on("change", function() {
            var value = jQuery(this).val() + "px";
            jQuery(
               "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?> .twss_6310_team_style_<?php echo $ids; ?>_read_more div { font-size:" +
               value + ";} </style>").appendTo(".twss-6310-preview");
         });
         ...
         jQuery("#twss_read_more_font_color").on("change", function() {
            var value = jQuery(this).val();
            jQuery(
               "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?> .twss_6310_team_style_<?php echo $ids; ?>_read_more div { color:" +
               value + ";} </style>").appendTo(".twss-6310-preview");
         });
         ...
         jQuery("#twss_read_more_border_color , #twss_read_more_border_width").on("change", function() {
            ...
            jQuery(
               "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?> .twss_6310_team_style_<?php echo $ids; ?>_read_more div { border:" +
               border_width + " solid " + border_color + ";} </style>").appendTo(".twss-6310-preview");
         });
         jQuery("#twss_read_more_border_hover_color , #twss_read_more_border_width").on("change", function() {
            var border_color = jQuery("#twss_read_more_border_hover_color").val();
            var border_width = jQuery("#twss_read_more_border_width").val();
            jQuery(
               "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?>:hover .twss_6310_team_style_<?php echo $ids; ?>_read_more div { border:" +
               border_width + " solid " + border_color + ";} </style>").appendTo(".twss-6310-preview");
         });
         ...
         jQuery("#twss_read_more_font_hover_color").on("change", function() {
            var value = jQuery(this).val();
            jQuery(
               "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?>:hover #twss_6310_team_style_<?php echo $ids; ?>_text  { color:" +
               value + ";} </style>").appendTo(".twss-6310-preview");
         });
         ...
         jQuery("#twss_read_more_border_radius").on("change", function() {
            var value = jQuery(this).val() + "px";
            jQuery(
               "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?> .twss_6310_team_style_<?php echo $ids; ?>_read_more div{ border-radius:" +
               value + ";} </style>").appendTo(".twss-6310-preview");
         });
         ...
         jQuery("#twss_read_more_backbround_color").on("change", function() {
            var value = jQuery(this).val();
            jQuery(
               "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?> .twss_6310_team_style_<?php echo $ids; ?>_read_more div { background-color:" +
               value + ";} </style>").appendTo(".twss-6310-preview");
         });
         ...
         jQuery("#twss_read_more_backbround_hover_color").on("change", function() {
            var value = jQuery(this).val();
            jQuery(
               "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?>:hover .twss_6310_team_style_<?php echo $ids; ?>_read_more div { background:" +
               value + " !important;} </style>").appendTo(".twss-6310-preview");
         });
         ...
         jQuery("#twss_read_more_font_weight").on("change", function() {
            var value = jQuery(this).val();
            jQuery(
               "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?> .twss_6310_team_style_<?php echo $ids; ?>_read_more div { font-weight:" +
               value + ";} </style>").appendTo(".twss-6310-preview");
         });
         ...
         jQuery("#twss_read_more_text_transform").on("change", function() {
            var value = jQuery(this).val();
            jQuery(
               "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?> .twss_6310_team_style_<?php echo $ids; ?>_read_more div { text-transform:" +
               value + ";} </style>").appendTo(".twss-6310-preview");
         });
         ...
         jQuery("#twss_jquery_read_more_font_family").on("change", function() {
            var value = jQuery(this).val();
            jQuery(
               "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?> .twss_6310_team_style_<?php echo $ids; ?>_read_more div { font-family:" +
               value + ";} </style>").appendTo(".twss-6310-preview");
         });
         ...
         jQuery("#twss_read_more_margin_top").on("change", function() {
            var value = jQuery(this).val();
            jQuery(
               "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?> .twss_6310_team_style_<?php echo $ids; ?>_read_more div { margin-top:" +
               value + "px !important;} </style>").appendTo(".twss-6310-preview");
         });
         ...
         jQuery("#twss_read_more_margin_bottom").on("change", function() {
            var value = jQuery(this).val();
            jQuery(
               "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?> .twss_6310_team_style_<?php echo $ids; ?>_read_more div { margin-bottom:" +
               value + "px !important;} </style>").appendTo(".twss-6310-preview");
         });
         ...
         jQuery("#twss_read_more_text_align").on("change", function() {
            ...
            if (value == 'left') {
               jQuery(
                  "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?> .twss_6310_team_style_<?php echo $ids; ?>_read_more div {margin: 0 auto 0 0 !important;} </style>"
               ).appendTo(".twss-6310-preview");
            } else if (value == 'center') {
               jQuery(
                  "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?> .twss_6310_team_style_<?php echo $ids; ?>_read_more div {margin: 0 auto !important;} </style>"
               ).appendTo(".twss-6310-preview");
            } else if (value == 'right') {
               jQuery(
                  "<style type='text/css'>.twss_6310_team_style_<?php echo $ids; ?> .twss_6310_team_style_<?php echo $ids; ?>_read_more div {margin: 0 0 0 auto !important;} </style>"
               ).appendTo(".twss-6310-preview");
            }
         });
      </script>
   ...
   function twss_fun_template_social($value, $cssData, $icon_table, $id, $template_id)
   {
      global $wpdb;
      if($value['iconids']){
         $icons = $wpdb->get_results($wpdb->prepare("SELECT * FROM $icon_table WHERE id in (%s)", $value['iconids']), ARRAY_A);
         $id = $template_id ? $template_id : $id;
         ...
         if ($icons) {
            ...
            $iconStyles = "";
            $total = isset($cssData['social_icon_number']) ? $cssData['social_icon_number'] : 4;
            foreach ($icons as $li) {
               $index = array_search($li['id'], $iconIds);
               if ($c == 0) {
               ...
               $c++;
               echo "<div><a ".twss_6310_external_link($iconUrl[$index])." class='open_in_new_tab_class' title='{$li['name']}' target='_blank' id='twss-social-link-{$value['id']}-{$li['id']}'><i class='" . $li['class_name'] . "'></i></a></div>";
   ...
   ?>
      <script>
         <?php
            if(!$cssData['twss_fun_template_social']){
         ...
         jQuery("body").on("click", ".activate-social", function() {
            ...
            if (val == 0) {
               jQuery(".social_act_field, .twss_6310_team_style_<?php echo $id; ?>_social").hide();
            } else {
               jQuery(".social_act_field, .twss_6310_team_style_<?php echo $id; ?>_social").show();
            }
         });
      </script>
   <?php
   ...
   function twss_6310_social_icon($iconId, $url, $borderWidth, $memberId, $templateId, $contactInfo = null, $styleId = null, 	$totalIcon = 4){
		global $wpdb;
		$icon_table = $wpdb->prefix . 'twss_6310_icons';
		$iconStyles = '';
		$c = 0;
		...
		if($iconId) {
			...
			if($iconIds && $iconUrl){
				for($i = 0; $i < count($iconUrl); $i++){
					if($iconIds[$i] != "" && $iconUrl[$i] != ""){
						if ($c == 0) {
							echo "<ul class='twss_6310_team_style_{$templateId}_social'>";
							if($contactInfo && $styleId){
								twss_6310_extract_contact_info($contactInfo, $styleId);
							}
						}
						
						$selIcon = $wpdb->get_row($wpdb->prepare("SELECT * FROM $icon_table WHERE id=%d", $iconIds[$i]), ARRAY_A);
						if($selIcon){
							echo "<li><a ".twss_6310_external_link($iconUrl[$i])."  title='{$selIcon['name']}'  id='twss-social-link-{$memberId}-{$selIcon['id']}'><i class='" . $selIcon['class_name'] . "'></i></a></li>";
							$iconStyles .= "<style>#twss-social-link-{$memberId}-{$selIcon['id']}{border: {$borderWidth}px solid {$selIcon['bgcolor']}; background-color: {$selIcon['bgcolor']}; color:{$selIcon['color']};} #twss-social-link-{$memberId}-{$selIcon['id']}:hover{color: {$selIcon['bgcolor']}; background-color:{$selIcon['color']};} </style>";
							$c++;
   ...
   function twss_fun_template_contact($contacts, $cssData, $id, $template_id = 0)
   {
      ...
      if ($contacts) {
         ...
         foreach ($contacts as $contact) {
            ...
            if (filter_var($contact[1], FILTER_VALIDATE_EMAIL)) {
               $dataAttr = "twss-data-custom-field='mailto:{$contact[1]}'";
               $dataClass = ' twss-6310-custom-field-mail-link-class';
            } else if (filter_var($contact[1], FILTER_VALIDATE_URL)) {
               $contact1 = twss_6310_validate_profile_url($contact[1]);
               $dataAttr = "twss-data-custom-field='{$contact[1]}'";
               $dataClass = ' twss-6310-custom-field-mail-link-class';
            }
            ...
            $labelText = str_replace("\\", "", $contact[0]);
            if(substr($labelText, 0, 7) == 'fab fa-' || substr($labelText, 0, 7) == 'fas fa-') {
               echo "<div class='twss-custom-fields-list{$dataClass}' {$dataAttr}><i class='{$labelText}'></i>{$contact[1]}</div>";
            } else {
               echo "<div class='twss-custom-fields-list{$dataClass}' {$dataAttr}><span>{$labelText}:</span>{$contact[1]}</div>";
            }
   ...
   ?>
      <script>
         <?php
            if(!$cssData['twss_fun_template_contact']){
         ...
         jQuery("body").on("click", ".activate-contact", function() {
            ...
            if (val == 0) {
               jQuery(".contact_act_field, .twss_6310_team_style_<?php echo $id; ?>_contact").hide();
            } else {
               jQuery(".contact_act_field, .twss_6310_team_style_<?php echo $id; ?>_contact").show();
            }
         });
      </script>
      <style>
         .twss-custom-fields-list i{
            margin-right: 4px;
         }
      </style>
   <?php
   ...
   ?>
      <div class="twss_6310_member_skills_wrapper_<?php echo $id; ?>">
         ...
         foreach ($skills as $skill) {
            ...
            $skill = explode("||||", $skill);
         ?>
            <div class="twss_6310_skills_label_<?php echo $id; ?>"><?php echo twss_6310_replace($skill[0]) ?></div>
            <div class="twss_6310_skills_prog_<?php echo $id; ?>">
               <div class="twss_6310_fill_<?php echo $id; ?> fill-<?php echo $id . "-{$skl}-$output" ?>" data-progress-animation="<?php echo $skill[1] ?>%" data-appear-animation-delay="400" style="width: <?php echo twss_6310_replace($skill[1]) ?>%;"></div>
            </div>
            <style>
               .fill-<?php echo $id . "-{$skl}-$output" ?> {
                  animation: mymove-<?php echo $id . "-{$skl}-$output" ?> 3s linear <?php echo ($cssData['progress_bar_animation']) ? 'infinite' : '';
      ...
      <script>
         <?php
            if(!$cssData['twss_fun_template_skills']){
...
<?php
   ...
   function twss_team_with_skills_and_slider_install()
   {
      ...
      if (!($catData)) {
			$wpdb->query("INSERT INTO {$category_table} (name, c_name, serial) VALUES
				...
				('Sales', 'c-1588100457', '2'),
            ('Support', 'c-1588101163', '3'),
            ('Admin', 'c-8988101189', '4')
         ");
      }

      $memberData = $wpdb->query("select id from $member_table LIMIT 1");
      if (!($memberData)) {
         $url = twss_6310_plugin_dir_url;
         ...
         $wpdb->query("INSERT INTO {$member_table} 
         (name, designation, profile_details_type, profile_url, open_new_tab, profile_details, effect, image, image_hover, iconids, iconurl, skills, contacts, category) 
         VALUES
            ('Margaret Thatcher', 'Sales Executive', '2', '', '0', '{$details}', 'top', '{$url}assets/image/1.jpg', '{$url}assets/image/1_hover.jpg', '3', 'https://www.facebook.com', 'Confidence||||95', 'email||||admin@example.com', 'c-1588100457 c-8988101189'),
				('Margaret Thatcher', 'Sales Executive', '2', '', '0', '{$details}', 'top', '{$url}assets/image/2.jpg', '{$url}assets/image/2_hover.jpg', '3', 'https://www.facebook.com', 'Confidence||||95', 'email||||admin@example.com', 'c-1588100457 c-1588101163'),
            ('Shawn Williams', 'Sales Executive', '2', '', '0', '{$details}', 'top', '{$url}assets/image/3.jpg', '{$url}assets/image/3_hover.jpg', '2', 'https://www.twitter.com', 'Confidence||||95', 'email||||admin@example.com', 'c-8988101189 c-1588101163'),
            ('Anthony Luiz', 'Account Executive', '2', '', '0', '{$details}', 'top', '{$url}assets/image/4.jpg', '{$url}assets/image/4_hover.jpg', '14', 'https://www.youtube.com', 'Confidence||||95', 'email||||admin@example.com', 'c-1588100457 c-8988101189'),
            ('Reenu Mathews', 'Sales Executive', '2', '', '0', '{$details}', 'top', '{$url}assets/image/5.jpg', '{$url}assets/image/5_hover.jpg', '1', 'https://www.linkedin.com', 'Confidence||||95', 'email||||admin@example.com', 'c-1588100457 c-8988101189')            
         ");
      }
   }

   function twss_6310_fa_icon_list($startTag, $endTag) {
      $iconArray = array (
   ...
   function twss_6310_delete_member_from_category_info($id){
      global $wpdb;
      $style_table = $wpdb->prefix . 'twss_6310_style';
   ...
   function twss_6310_update_all_category_member_info($id, $selCategory){
      global $wpdb;
      $category_table = $wpdb->prefix . 'twss_6310_category';
      $style_table = $wpdb->prefix . 'twss_6310_style';
   ...
   function twss_6310_extract_members($members, $styleId = 0)
   {
      global $wpdb;
      $member_table = $wpdb->prefix . 'twss_6310_member';
      $results = array();
      ...
      if ($members) {
         ...
         if ($memberids) {
            ...
            foreach ($memberids as $memid) {
               if ($memid) {
                  $tempMem = $wpdb->get_row($wpdb->prepare("SELECT * FROM $member_table WHERE id=%d", $memid), ARRAY_A);
                  if($tempMem){
                     $results[] = $tempMem;
                  }
               }
...
function twss_6310_category_menu($categoryData, $styleId, $output = '')
{
	echo "<div id='twss-6310-category-{$styleId}'>";
	$index = 1;
	$class = $output ? "twss_6310_category_list twss_6310_category_list_{$styleId}":'twss_6310_category_list';
	foreach($categoryData as $value){
		$active = ($index)? ($output ? " twss_6310_category_list_active_{$styleId}": " twss_6310_category_list_active") :'';
		echo "<div class='{$class}{$active}' twss-data-filter='{$value['c_name']}'>{$value['name']}</div>";
		$index = 0;
	}
	echo "</div>";
}
...
function twss_6310_check_field_exists() {
